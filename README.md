...
Use `ssb_search()` to find tables by keyword. The function returns a
data frame with one row per matching table; the `id` column is what you
...
Use `ssb_codes()` to list the valid values you can use as filters for a
given dimension:
...
Pass a table identifier to `get_ssb_data()` to download the full table.
SSB enforces a cell limit on large tables; the function will warn you
and stop if the estimated row count is too high.
...
By default, `get_ssb_data()` converts quarterly time codes to
`zoo::yearqtr` and monthly time codes to `zoo::yearmon`.
...
`output_values` arguments to `get_ssb_data()`:
...
Or use the convenience wrapper `ssb_get_by_codelist()` which identifies
the correct dimension automatically:
...
`cache = FALSE` to disable caching, or `refresh_metadata = TRUE` to
discard the cached copy and fetch fresh metadata:
...
result size before sending the request and stops with a message if the
query is too large. To proceed anyway, set
